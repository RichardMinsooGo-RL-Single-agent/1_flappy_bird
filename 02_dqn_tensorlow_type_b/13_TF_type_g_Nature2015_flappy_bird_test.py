...
class DQN_agent:
    ...
    def preprocess(self, state):
        state = skimage.color.rgb2gray(state)
        state = skimage.transform.resize(state_out,(self.img_rows,self.img_cols))
        state = skimage.exposure.rescale_intensity(state_out, out_range=(0, 255))
        state = state / 255.0
        
        # state = state.reshape(1, state.shape[0], state.shape[1], 1) #1x80x80x1
...
def main():
    ...
    while time.time() - start_time < agent.training_time:
        ...
        while not done and ep_step < agent.ep_trial_step:
            ...
            stacked_next_state[:,:,0:3] = copy.deepcopy(stacked_state[:,:,1:4])
            stacked_next_state[:,:,3]   = next_state
